...
for (i in c(1:8)){
	a <- paste("IC", i, sep = "")
	plot(x = train[[3]]$time, y = train_ics_original[[3]][,get(a)], type = "l", ylim = c(-10,10))
}


train_ics_original_permuted <- vector("list", 36)
...
for (i in c(1:36)){
	...
	for (j in c(1:8)){
		...
		ica_signal <- as.integer(substr(zdf$V2[j], 3,3))
		
		permutation_matrix[ica_signal,original] <- 1
	}
	...
	colnames(train_ics_original_permuted[[i]]) <- c("IC1", "IC2", "IC3", "IC4", "IC5", "IC6", "IC7", "IC8", "time", "class")
	
}


a <- cor(train[[4]], train_ics_original[[4]])
zdf <- as.data.table(as.table(abs(a)))
zdf <- zdf[order(-rank(N))][1:8]
zdf
...
for (i in c(1:length(train))){
	pat <- train[[i]]
	ica_pat <- train_ics_original_permuted[[i]]
	signal_end <- 0
